...
int main(){
	...
	if((bind(server_socket,(struct sockaddr *)&server,sizeof(struct sockaddr_in)))==-1){perror("bind error: ");exit(-1);} //binds the socket to the sockaddr_in conatining ip address and port
	if((listen(server_socket,8))==-1){perror("listen error: ");exit(-1);}//tells the socket to listen for an incomming signal
	...
	while(true){
		...
		seed++;
		thread t(handle_client,client_socket,seed); //creates a thread for the client
